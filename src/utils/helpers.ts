import { LoanArgs, MarketArgs, ReserveArgs } from "./modals";
import {
...
export async function getReserveRewardsApy(args: ReserveArgs) {
  ...
  const oraclePrices = await new Scope(
    "mainnet-beta",
    args.connection
  ).getOraclePrices();
...
export function calculateRewardApy(
  ...
  const rewardsInYearValue = rewardsInYear.mul(
    prices.spot[rewardInfo.token.mint.toString()].price
  );
